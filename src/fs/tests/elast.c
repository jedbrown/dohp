...
static void ElastExact_3_Solution(const struct ElastExactCtx *ctx,const dReal x[3],dScalar u[3],dScalar du[9]) {
  u[0] = 0;
  u[1] = 0;
  u[2] = ctx->a*x[2];
  PetscMemzero(du,9*sizeof(dScalar));
}
static void ElastExact_3_Forcing(const dUNUSED struct ElastExactCtx *ctx,const dUNUSED struct ElastParam *param,const dUNUSED dReal x[3],dScalar f[3]) {
  f[0] = 0;
  f[1] = 0;
  f[2] = 0;
}
...
static dErr ElastSetFromOptions(Elast elt)
{
  ...
  switch (exact) {
    ...
    case 2:
      ...
      break;
    case 3:
      elt->exact.solution = ElastExact_3_Solution;
      elt->exact.forcing = ElastExact_3_Forcing;
      break;
